...
namespace MP.Common
{
    public class IPWryHelper
    {
        #region 私有成员

        // 地址数据库文件所在路径
        private string dataPath = System.Web.HttpContext.Current.Server.MapPath("/bin/QQWry.Dat");
        private string ip;
        ...
        public string DataPath { set { dataPath = value; } }
        ...
        public string IPLocation()
        {
            ...
            return getIPLocation(this.country, this.local, true);
        }
        ...
        private string getIPLocation(string country, string local, bool isIgnore)
        {
            string IPLocation = "";
            IPLocation = country + " " + local;

            if (isIgnore)
            {
                if (IPLocation.IndexOf("保留地址") > 0 || country.IndexOf("IANA") >= 0)
                {
                    IPLocation = "中国上海";
                }
            }
